# multi_model_tests_with_StageI_all_poolings_fixed_threshold.py
...
def main():
    ...
    # Testing: run all pooling methods and compare with fixed threshold
    headers = ['Pooling Method', 'ROC-AUC', 'Optimal Threshold (ROC)', 'Overall Acc', 'Stage I Acc', 'Stage II Acc']
    overall_results = []
    ...
    for i, model in enumerate(models):
        ...
        for method in POOL_METHODS:
            ...
            # Compute ROC-AUC and ROC curve (for plotting)
            try:
                auc_val = float(roc_auc_score(labels_pt.numpy(), scores_pt.numpy()))
            except Exception:
                auc_val = float('nan')
            try:
                fpr, tpr, roc_thresh = roc_curve(labels_pt.numpy(), scores_pt.numpy())
            except Exception:
                fpr, tpr, roc_thresh = None, None, None

            # Fixed-threshold predictions and accuracies
            preds = (scores_pt.numpy() > TEST_THRESHOLD).astype(int)
            ...
            # Stage masks aligned with test_pts ordering
            stageI_mask = np.array([1 if pt in stageI_pts else 0 for pt in test_pts], dtype=bool)
            stageII_mask = ~stageI_mask

            # If FAST_TEST truncated test_pts, masks still align because patient_wise_loader_outputs used same test_pts
            # But guard lengths
            if len(stageI_mask) != len(labels_np):
                # build mask from patient indices used when generating scores
                # simpler fallback: inspect saved CSV to recover patient_index column (already saved)
                csv_path = f'outputs/{model.name}/image_outputs_patientwise_pool-{method}.csv'
                if os.path.exists(csv_path):
                    df = pd.read_csv(csv_path)
                    patient_indices_order = df['patient_index'].astype(int).tolist()
                    stageI_mask = np.array([1 if idx in stageI_pts else 0 for idx in patient_indices_order], dtype=bool)
                    stageII_mask = ~stageI_mask
                else:
                    # fallback: assume all test patients are Stage II if mismatch (unlikely)
                    stageI_mask = np.zeros(len(labels_np), dtype=bool)
                    stageII_mask = ~stageI_mask

            # Stage I metrics
            stageI_indices = np.where(stageI_mask)[0]
            stageII_indices = np.where(stageII_mask)[0]

            def acc_on_indices(idxs):
                if len(idxs) == 0:
            ...
            s1_correct, s1_total, s1_acc = acc_on_indices(stageI_indices)
            s2_correct, s2_total, s2_acc = acc_on_indices(stageII_indices)
            ...
            try:
                if fpr is not None and tpr is not None:
                    ...
                    ax.set_xlabel('FPR')
                    ax.set_ylabel('TPR')
                    ax.set_title(f'{model.name} - ROC ({method})')
                    ax.legend()
                    fig.tight_layout()
                    fig.savefig(fname)
                    plt.close(fig)
            ...
            # Text log for model
            with open(f'outputs/{model.name}/results.txt', 'a') as f:
                f.write(f'\n>>> {model.name} patient-wise test (pool={method}) with fixed threshold={TEST_THRESHOLD}\n')
                f.write(f'ROC-AUC: {auc_val:.4f}\n')
                f.write(f'Overall: {overall_correct}/{overall_total} correct -> {overall_acc:.4f}\n')
                f.write(f'Stage I: {s1_correct}/{s1_total} correct -> {s1_acc if not np.isnan(s1_acc) else "N/A"}\n')
                f.write(f'Stage II: {s2_correct}/{s2_total} correct -> {s2_acc if not np.isnan(s2_acc) else "N/A"}\n')
                f.write('----------------------------------------------------\n')
            ...
            per_model_results.append({
                ...
                'pool_method': method,
                'roc_auc': auc_val,
                'opt_threshold_roc': float(roc_thresh[ np.argmax(tpr - fpr) ]) if (roc_thresh is not None and len(roc_thresh)>0) else float('nan'),
                'overall_correct': overall_correct,
                'overall_total': overall_total,
                'overall_acc': overall_acc,
                'stageI_correct': s1_correct,
                'stageI_total': s1_total,
                'stageI_acc': s1_acc,
                'stageII_correct': s2_correct,
                'stageII_total': s2_total,
                'stageII_acc': s2_acc
            })

            # Also print detailed block to console
            print(f'\nPOOL METHOD: {method}')
            print(f'  ROC-AUC: {auc_val:.4f}')
            print(f'  Overall: {overall_correct}/{overall_total} -> {overall_acc:.4f}')
            print(f'  Stage I: {s1_correct}/{s1_total} -> {s1_acc if not np.isnan(s1_acc) else "N/A"}')
            print(f'  Stage II: {s2_correct}/{s2_total} -> {s2_acc if not np.isnan(s2_acc) else "N/A"}')

        # Save per-model pooling summary CSV and consolidated table
        if len(per_model_results) > 0:
            ...
            # Also save a concise comparison CSV per model
            comp_csv = f'outputs/{model.name}/pooling_accuracy_comparison.csv'
            comp_df = summary_df[['pool_method', 'overall_acc', 'stageI_acc', 'stageII_acc', 'roc_auc']]
            comp_df.to_csv(comp_csv, index=False)

            # Print a neat table block
    ...
    if len(overall_results) > 0:
        ...
        human_cols = ['model', 'pool_method', 'roc_auc', 'overall_correct', 'overall_total', 'overall_acc',
