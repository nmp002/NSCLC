...
def main():
    ...
    model_files = [
        ...
        if f.startswith("2-Planed") # change on model-to-model basis
    ]
    ...
    for model_file in model_files:
        ...
        thr_train = scores.get("Optimal Threshold from ROC", 0.4724)
        ...
        # ============================================================
        # STAGE II ONLY @ CONSTANT THRESHOLD 0.4724
        # ============================================================

        CONST_THR = 0.4724 # can change based on optimized-training threshold

        TEST_SETS = {
            "const_thr0p4724_stageII": TEST_PTS_STAGEII,
        }
        ...
        for test_name, pt_list in TEST_SETS.items():
            ...
            ax.set_title(f"Confusion Matrix @0.4724\n({test_name})")

            for (i, j), v in np.ndenumerate(cm):
                ax.text(j, i, str(v), ha="center", va="center")

            fig_cm.savefig(os.path.join(out_test, "confusion_matrix_thr0p4724.png"))
            plt.close(fig_cm)
            ...
            with open(os.path.join(out_test, "results_thr0p4724.txt"), "w") as f:
                f.write(f"Model: {model_name}\n")
