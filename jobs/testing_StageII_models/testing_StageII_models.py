...
def main():
    ...
    for model_file in model_files:
        ...
        thr_train = scores.get("Optimal Threshold from ROC", 0.3358)
        ...
        for test_name, pt_list in TEST_SETS.items():
            ...
            ax.set_title(f"Confusion Matrix @0.3358\n({test_name})")

            for (i, j), v in np.ndenumerate(cm):
                ax.text(j, i, str(v), ha="center", va="center")

            fig_cm.savefig(os.path.join(out_test, "confusion_matrix_thr0p3358.png"))
            plt.close(fig_cm)
            ...
            with open(os.path.join(out_test, "results_thr0p3358.txt"), "w") as f:
                f.write(f"Model: {model_name}\n")
