...
def main():
    ...
    model_files = [
        ...
        if f.startswith("3-Planed ResNet18_nadh_fad_orr_lr_1e-07_wd_0.2_") # change on model-to-model basis
    ]
    ...
    for model_file in model_files:
        ...
        thr_train = scores.get("Optimal Threshold from ROC", 0.7137)
        ...
        # ============================================================
        # STAGE II ONLY @ CONSTANT THRESHOLD 0.7137
        # ============================================================

        CONST_THR = 0.7137 # can change based on optimized-training threshold

        TEST_SETS = {
            "const_thr0p7137_stageII": TEST_PTS_STAGEII,
        }
        ...
        for test_name, pt_list in TEST_SETS.items():
            ...
            ax.set_title(f"Confusion Matrix @0.7137\n({test_name})")

            for (i, j), v in np.ndenumerate(cm):
                ax.text(j, i, str(v), ha="center", va="center")

            fig_cm.savefig(os.path.join(out_test, "confusion_matrix_thr0p7137.png"))
            plt.close(fig_cm)
            ...
            with open(os.path.join(out_test, "results_thr0p7137.txt"), "w") as f:
                f.write(f"Model: {model_name}\n")
