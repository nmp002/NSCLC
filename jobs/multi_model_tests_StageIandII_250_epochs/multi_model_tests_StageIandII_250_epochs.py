# stageII_fixed_split_stageI_test.py
# Train on fixed 17 Stage II patients, test on 8 Stage II + 13 Stage I.
# Threshold is ROC-optimized on Stage II test set.
...
TOTAL_EPOCHS = 250          # main request: train for 250 epochs
...
def main():
    ...
    train_loader = torch.utils.data.DataLoader(train_set,
                                               ...
                                               shuffle=True,
                                               num_workers=0)
    test_loader_stageII = torch.utils.data.DataLoader(test_set_stageII,
                                                      batch_size=batch_size,
                                                      shuffle=False,
                                                      num_workers=0)
    ...
    for (m, tl, ta) in zip(models, train_loss, train_auc):
        df = pd.DataFrame({'Training Loss': tl, 'Training ROC-AUC': ta},
                          index=range(1, total_epochs + 1))
        df.to_csv(f'outputs/{m.name}/tabular_train.csv', index_label='Epoch')

        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(16, 5))
        plt.suptitle(m.name)
        ax1.plot(df.index, df['Training Loss'])
        ax1.set_xlabel('Epoch'); ax1.set_ylabel('Loss'); ax1.set_title('Training Loss')
        ax2.plot(df.index, df['Training ROC-AUC'])
        ax2.set_xlabel('Epoch'); ax2.set_ylabel('AUC'); ax2.set_title('Training ROC-AUC')
        fig.savefig(f'outputs/{m.name}/plots/loss_auc_curves.png')
        plt.close(fig)
    ...
    for pool_method in POOL_METHODS:
        ...
        for m in models:
            ...
            with open(f'outputs/{m.name}/results.txt', 'a') as f:
                f.write(f'\n[TEST Stage II] {m.name} pooling={pool_method}\n')
                f.write(f'ROC-opt threshold from Stage II test: {format_metric(thr_test)}\n')
            ...
            with open(f'outputs/{m.name}/results.txt', 'a') as f:
                f.write(f'\n[TEST Stage I] {m.name} pooling={pool_method} (threshold from Stage II test)\n')
